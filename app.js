...
function renderRanking() {
  ...
  container.innerHTML = sorted.map((c, i) => {
    ...
    const barColor = inTop
      ...
      : '#1E3A58';
    const surnameColor = inTop ? '#C8DFF8' : '#4A6A88';
    const numColor     = inTop ? '#F5C518' : '#3A5878';
    const ptsColor     = inTop ? '#7AABDC' : '#3A5878';
    ...
    const divider  = i === topN
      ? `<div class="lrp-divider"><span>— ibaba ng top ${topN} —</span></div>`
      : '';
    return `${divider}
      <div class="lrp-row ${rankCls} ${topCls}">
        <span class="lrp-num" style="color:${numColor}">${i+1}</span>
        ...
        <div class="lrp-bar-col">
          <span class="lrp-surname" style="color:${surnameColor}">${surname}</span>
          <div class="lrp-track">
            <div class="lrp-fill" style="width:${pct}%;background:${barColor}"></div>
          </div>
        </div>
        <span class="lrp-pts" style="color:${ptsColor}">${pts}pt</span>
      </div>`;
  }).join('');
}
