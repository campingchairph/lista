...
function renderQuestion() {
  ...
  badge.textContent = q.bad ? 'Negatibong katangian' : 'Positibong katangian';
    ...
    return `
      <div class="cand-item ${cls}" id="ci-${c.id}" onclick="App.toggleCand('${c.id}')">
        ...
        <div class="cand-details">
          ...
          <div class="cand-role">Kandidato — ${ROLES.find(r => r.id === role)?.label}</div>
        </div>
      </div>`;
  }).join('');
  ...
  document.getElementById('q-body').innerHTML = `
    <div class="q-card">
      <div class="q-num">TANONG ${qi + 1} &nbsp;·&nbsp; KATEGORYA ${q.cat} — ${q.catLabel}</div>
    ...
    <p class="q-hint">Pwedeng marami, pwedeng wala — ikaw ang humusga</p>
